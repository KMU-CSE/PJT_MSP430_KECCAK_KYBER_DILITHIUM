...
static void inner_matacc(poly* res, poly* src, const int ctr, const uint16_t* coval, int16_t zeta)
{
	int16_t r[4];
        asm_basemul(&r[0], &coval[0], &src->coeffs[ctr - 4], zeta);
...
void matacc(poly* res, polyvec* src, unsigned char i, const unsigned char* seed, int transposed)
{
	...
	xof_state state;
	volatile int ctr = 0, pos = 0, k = 0;
	uint16_t val0, val1;
	int16_t coval[4] = {0};
	int16_t c[4]= {0}, tmp[4]= {0};
